var ticloudagent_version = '2.1.5107';
var me = window;
while (me.parent !== me) {
    me = me.parent;
}
...
(function (TICloudAgent) {
    ...
    var MissingExtension = (function (_super) {
        ...
        function MissingExtension() {
            var _this = _super.call(this, "MissingExtension", "MISSING_EXTENSION") || this;
    ...
    // returns a promise that either rejects with a MissingExtension
    // or LocationNotSupported error array (depending on location)
    function getExtensionError() {
        return isLocationSupported().then(function (locationValid) {
            if (locationValid) {
                throw new MissingExtension();
            }
            throw new LocationNotSupported();
        });
    }
    ...
    // The typo is intentional, as existing and pre-installed extensions use this string and we don't have a good way 
    // of updating agent.js and the extension in unison...
    var EXT_MSG_TYPES = {
    ...
    var Extension = (function () {
        ...
        Extension.prototype.registerInitMessageHandler = function (deferred) {
            var _this = this;
            this.regOnMessage(function (msgObj) {
                if (msgObj.type === EXT_MSG_TYPES.INIT_COMPLETE) {
                    _this.removeLastOnMessage(); // stop listening to the init event
    ...
    var ChromeBaseExtension = (function (_super) {
        ...
        ChromeBaseExtension.prototype.init = function () {
            // If our extension is not installed/enabled, then chrome.runtime may not be defined.
            var _this = this;
            if (!chrome.runtime || !chrome.runtime.connect) {
                return getExtensionError();
            }
            // Call chrome.runtime.connect with the extension id to connect.  If it fails to connect, the onDisconnect
            // listener will be called.  However, this will happen sometime after we post the first message.
            var deferred = Q.defer();
            this.port = chrome.runtime.connect(this.ID);
            this.port.onDisconnect.addListener(function () {
                _this.port = null;
                getExtensionError().catch(function (e) { return deferred.reject(e); });
            });
            // Setup a listener to get and return the initial init response			
            this.registerInitMessageHandler(deferred);
            // Send the init message
            // Chrome documentation states this will throw if the extension is not installed, but my testing has shown 
            // that's not quite right.  There appears to be a race condition (you can see it by setting breakpoints in 
            // this function).  If another extension is installed with messaging (meaning chrome.runtime is defined), 
            // then postMessage may throw, or it may not, but in the latter case onDisconnect is called.
            try {
                this.postMessage({ type: EXT_MSG_TYPES.INIT });
            }
            catch (e) {
                return getExtensionError();
            }
        ...
        ChromeBaseExtension.prototype.regOnMessage = function (listener) {
            this.lastRegListener = listener;
            this.port.onMessage.addListener(this.lastRegListener);
        };
        ChromeBaseExtension.prototype.removeLastOnMessage = function () {
            this.port.onMessage.removeListener(this.lastRegListener);
        };
    ...
    var ChromeExt = (function (_super) {
        ...
        function ChromeExt() {
            ...
            // cspell: disable-next-line
            "pfillhniocmjcapelhjcianojmoidjdk", "https://chrome.google.com/webstore/detail/") || this;
        }
        return ChromeExt;
    }(ChromeBaseExtension));
    // Edge ext specific
    var EdgeExt = (function (_super) {
        ...
        function EdgeExt() {
            return _super.call(this, 
            // cspell: disable-next-line
            "ockfhepjmcpmlejemmjokgoepkopgegd", "https://microsoftedge.microsoft.com/addons/detail/ticloudagent-bridge/") || this;
        }
        return EdgeExt;
    }(ChromeBaseExtension));
    var DOMEventExt = (function (_super) {
        ...
        DOMEventExt.prototype.init = function (version) {
            // DOM based extensions are booted up at the start and are thus already initialized.  Simply check if it
            // was successful.  Search the code for TICLOUDAGENTBRIDGE-BOOT-REPLY to see how this works
            if (!this.isInstalled()) {
                return getExtensionError();
            }
            // Check if the version is valid
            if (this.getVersion() !== version) {
                return Q.reject(new InvalidExtensionVersion());
            }
            // Setup a listener to get and return the initial init response
            var deferred = Q.defer();
            this.registerInitMessageHandler(deferred);
        ...
        DOMEventExt.prototype.getVersion = function () {
            return extensionInfo.version;
        };
        DOMEventExt.prototype.isInstalled = function () {
    ...
    function launchHostApp(agentConfig) {
        // abstraction for interfacing with the host app
        function initExtensionBased(extAgentConfig, browserType) {
            ...
            var SocketToExtension = (function () {
                ...
                SocketToExtension.prototype.close = function () {
                    ...
                    if (this.onclose) {
                        // The current ancient version of typescript has the wrong types for CloseEvent, so we cast to any
                        this.onclose(new CloseEvent("", { code: NORMAL_CLOSE, wasClean: true, reason: "Closed by client" }));
                    }
                };
            ...
            return browserExt.init(extAgentConfig[browserType].version)
                .then(function (msgObj) {
                // Check for an error from the extension with starting CA
                if (msgObj.data.error) {
                    throw new AgentNotStarted(msgObj.data.error);
                }
                // Check that the version of CA is correct
                if (msgObj.data.version !== extAgentConfig.installer.version ||
                    ...
                    throw new InvalidAgentVersion();
                }
                // Now register the the socket callback handler to handle communication with CA
                browserExt.regOnMessage(SocketToExtension.dispatchSocketEvent);
                return msgObj.data;
            })
                .catch(function (e) {
                // Historically, if we had an invalid extension, we'd also check the version of ti cloud agent.
                // This meant we would have an array of errors.  Going forward, I'm not supporting legacy 
                // extensions, but I'll keep returning an array in case there is code other than our error wizard
                // code that looks at this
                throw [e];
            });
        }
    ...
    var InstallWizard = (function () {
        ...
        InstallWizard.getDescriptionText = function (errors) {
            ...
            if (wizardDesc === "") {
                wizardDesc = "An unknown error has occurred when starting TI Cloud Agent.";
            }
            else {
    ...
    // Class to implement a simple event registration mechanism
    var SimpleEvent = (function () {
        function SimpleEvent() {
            this.listeners = [];
        }
        SimpleEvent.prototype.addListener = function (cb) {
            this.listeners.push(cb);
        };
        SimpleEvent.prototype.removeListener = function (cb) {
            var index = this.listeners.indexOf(cb);
            if (index !== -1) {
                this.listeners.splice(index, 1);
            }
        };
        SimpleEvent.prototype.dispatch = function () {
            // Clone the listeners array to avoid issues if listeners are added/removed during dispatch
            var listenersCopy = this.listeners.slice();
            for (var _i = 0, listenersCopy_1 = listenersCopy; _i < listenersCopy_1.length; _i++) {
                var listener = listenersCopy_1[_i];
                listener();
            }
        };
        SimpleEvent.prototype.hasListeners = function () {
            return this.listeners.length > 0;
        };
        return SimpleEvent;
    }());
    // This will fire "reconnecting" the first time a module starts reconnecting, and "reconnected" once all modules 
    // finish reconnecting.  
    var ModuleReconnectionMgr = (function () {
        function ModuleReconnectionMgr() {
            this.reconnecting = [];
            this.events = {
                reconnecting: new SimpleEvent(),
                reconnected: new SimpleEvent(),
            };
        }
        ModuleReconnectionMgr.prototype.addListener = function (event, cb) {
            this.events[event].addListener(cb);
        };
        ModuleReconnectionMgr.prototype.removeListener = function (event, cb) {
            this.events[event].removeListener(cb);
        };
        ModuleReconnectionMgr.prototype.addModule = function (module) {
            var _this = this;
            module.addListener("reconnecting", function () {
                _this.reconnecting.push(module);
                if (1 === _this.reconnecting.length) {
                    _this.events.reconnecting.dispatch();
                }
            });
            var onModuleDoneReconnecting = function () {
                var index = _this.reconnecting.indexOf(module);
                if (index !== -1) {
                    _this.reconnecting.splice(index, 1);
                    if (0 === _this.reconnecting.length) {
                        _this.events.reconnected.dispatch();
                    }
                }
            };
            module.addListener("reconnected", onModuleDoneReconnecting);
            module.addListener("close", onModuleDoneReconnecting); // Handles an intentional close mid-reconnect
        };
        ModuleReconnectionMgr.prototype.isActive = function () {
            return this.events.reconnecting.hasListeners() &&
                this.events.reconnected.hasListeners();
        };
        return ModuleReconnectionMgr;
    }());
    var reconnectionMgr = new ModuleReconnectionMgr();
    ...
    function createClientModule(port, subProtocol) {
        ...
        reconnectionMgr.addModule(moduleObj);
        ...
        function createCommandDispatch() {
            ...
            var wsReconnecting = "Module is Reconnecting";
            function exec(ws, commandName, data) {
                var promise = Q.defer();
                var obj = {
                    command: commandName,
                    id: commandID++,
            ...
            function ret(retObj) {
                ...
                if (pendingCommands[id]) {
                    ...
                    // delete it instead of nulling so the map doesn't grow too large over time
            ...
            var commandDispatchObj = { exec: exec, ret: ret };
            function cleanUp(message) {
                ...
                commandDispatchObj.exec = function () {
                    ...
                    defCommand.reject(new Error(message));
            ...
            // If we close or start re-connecting, we need to clean up all calls still waiting for a response, and 
            // start rejecting any new calls.  The only difference is the message.
            moduleObj.addListener("close", function () { return cleanUp(moduleClosedMsg); });
            moduleObj.addListener("reconnecting", function () { return cleanUp(wsReconnecting); });
            // Once we successfully re-connect, we need to re-hook up the exec/ret functions to handle calls normally.
            moduleObj.addListener("reconnected", function () {
                commandDispatch.exec = exec;
                commandDispatch.ret = ret;
            });
        ...
        // are we in the process of reconnecting this WebSocket
        var isReconnecting = false;
        function onclose(e) {
            // This is fired when the websocket closes, either intentionally, due to a crash, or due to chromium 
            // closing the WebSocket during a sleep or when idle.  If not intentional (ie an abnormal close), try to
            // re-establish the connection after a short timeout
            // If we got the close while re-connecting, that means we failed to reconnect.  If some UI client is 
            // listening to the reconnection manager, we can retry indefinitely as the user will be aware of what's 
            // happening, but if not we should close as otherwise user's will never see a debugger crash for what it is
            if ((isReconnecting && !reconnectionMgr.isActive()) || e.code !== ABNORMAL_CLOSE) {
                ...
                setTimeout(function () {
                    ws = subProtocol ? new Socket(socketUrl(), subProtocol) : new Socket(socketUrl());
                    ws.onclose = onclose;
                    ws.onerror = onerror;
                    ws.onopen = onopen;
                }, 1000);
        ...
        function onopen() {
            try {
                ...
                // handle close by sending an explicit close message and waiting for other side to close
                moduleObj.close = function () {
                    isReconnecting = true; // prevents an attempt to reconnect
                    var waitForClose = Q.defer();
                    moduleObj.addListener("close", function () { return waitForClose.resolve(); });
                    ws.send('{"command":"close"}');
                    return waitForClose.promise;
                };
                // if we are re-connecting, turn off the flag and fire a reconnected event.  The event will be picked 
                // up by the command dispatch to re-enable commands, so we need to do this BEFORE calling listCommands 
                // or listCommands will reject.  And, we need to call listCommands so all the command structures are
                // re-initialized with the new ws object.
                if (isReconnecting) {
                ...
                createCommand(ws, "listCommands"); // every module should have a listCommands command
    ...
    function Init() {
        if (!cachedInit) {
            cachedInit = loadAgentConfig()
                ...
                .then(function (agent) {
                agent.getReconnectionManager = function () { return reconnectionMgr; };
