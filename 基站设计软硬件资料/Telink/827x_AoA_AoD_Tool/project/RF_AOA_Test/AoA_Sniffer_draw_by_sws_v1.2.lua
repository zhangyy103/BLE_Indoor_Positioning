...
function RF_PER_Run_AOA(hDut)
	...
	local therta_aver_old = 0
    local ang_pos_or_neg_old = 0
	local input_angle = 0 
	local diff = array.new(8)
	local board_type = 2
	local IQ_data_len_8_bit = 0
	local payload_len = 0;
	--local para= {}
	seq =0xff
	local pkt_cnt=0
	...
	--pkt_addr = 0x41c9c --ÐèÒª ¸ù¾Ýcode±àÒë³öÀ´µÄbackup µØÖ·¸ÄÐ´
	position =1
	...
	rst_alg(hDut)
	command[1] = 0x33 
	Send_Cmd_To_DUT(hDut,command,16)
	if(Is_Result_Empty(hDut,1000)==1) then
		r, l = Read_Result_From_DUT(handle , 32)
		chn = r[2] + 2400
		accesscod = bit32.bor(bit32.lshift (r[6], 24), bit32.lshift (r[5], 16),bit32.lshift (r[4], 8),r[3])
		board_type = r[7]
		IQ_data_len_8_bit = r[8]
		payload_len = r[9]
	end
	...
	do
		...
		if save_en == 1 then
			...
			do
				...
				if(pkt_cnt<recode_pkt_num)then
					...
					para,l = Read_Result_From_DUT(hDut , payload_len+122)
					s= " "
					if(IQ_data_len_8_bit == 1)then 
					
						if((l==(payload_len+122)) and (para[1]==para[6]+14+90)) then
						
							rsquare = para[payload_len+112]*256 + para[payload_len+111] 
							input_angle =  para[payload_len+110]*256 + para[payload_len+111]
							if( (rsquare > 700) or (math.abs(input_angle)<5) or (math.abs(256 - input_angle)<5) or (board_type==2) ) then
								...
								for i=1,90,1 do
									if(para[i+payload_len+10]>128)then
										f:write(string.format("-%d ", 256-para[i+payload_len+10]))
									elseif(para[i+payload_len+10]<128)then
										f:write(string.format("%d ", para[i+payload_len+10]))
									else
								...
								f:write(string.format("%d \n",(para[payload_len+118])))	
								
									
								--5. record 130 recieve data
								for i=1,payload_len+112,1 do
								...
								--fback:write(string.format("\n"))
								--fback:write("RSSI :")
								--fback:write(string.format("%d ",(para[payload_len+107]-110)))
								...
								if(board_type == 1)then
									...
									fback:write(string.format("\n"))
								elseif(board_type == 2)then
									f:write(string.format("minval :"))
									minval = para[payload_len+113]*256*256*256 + para[payload_len+114]*256*256 + para[payload_len+115]*256 + para[payload_len+116] 
									f:write(string.format("%d \n",minval))
									f:write(string.format("\n"))
								end	
								...
								--10. print rsquare 
								rsquare = para[payload_len+113]*256*256*256 + para[payload_len+114]*256*256 + para[payload_len+115]*256 + para[payload_len+116] 
								print(string.format("rsquare: %d\n", rsquare))
								if(pkt_cnt>=recode_pkt_num)then
									local temp2 = array.new(1)
									temp2[1] = 0x80
									Core_write(hDut,0xf00,0x80)
									io.close(f)
									io.close(fback)
								end	
							end
						...
						if((l==(payload_len+122)) and (para[1]==bit32.band(para[6]+14+225,0xff))) then
							rsquare = para[payload_len+112]*256 + para[payload_len+111] 
							...
							if( (rsquare > 700) or (math.abs(input_angle)<5) or (math.abs(256 - input_angle)<5) or (board_type==2) ) then
								...
								elseif(board_type == 2)then
									f:write(string.format("minval :"))
									minval = para[payload_len+113]*256*256*256 + para[payload_len+114]*256*256 + para[payload_len+115]*256 + para[payload_len+116] 
									--minval = para[131]*256*256*256 + para[132]*256*256 + para[133]*256 + para[134] 
									f:write(string.format("%d \n",minval))
									f:write(string.format("\n"))
								end
		...
		elseif start_en==1 then
			...
			if board_type==1 then
				...
				tl_form_draw_line(180,245,180,170,0x008000)
				while(1)
				do
					para,l = Read_Result_From_DUT(hDut , payload_len+122)
					if(IQ_data_len_8_bit == 1) then 	
						if((l==payload_len+122) and (para[1]==para[6]+14+90)) then
							...
							rsquare = para[payload_len+112]*256 + para[payload_len+111] 
							input_angle =  para[payload_len+110]
							if( (rsquare > 700) or (math.abs(input_angle)<5) or (math.abs(256 - input_angle)<5)  ) then
								...
								---2.get parameter------------------------------------------------
								seq = para[8]
								rssi = para[payload_len+107]
								rx_cnt= rx_cnt+1
								angle_table[count] = input_angle
								rssi_table[count] = rssi
								therta_table[count]= para[payload_len+118]
			...
			elseif board_type==2 then
				...
				do
					...
					if(IQ_data_len_8_bit == 1) then 
						...
						if((l==payload_len+122) and (para[1]==para[6]+14+90) ) then	
							---1.get packet --------------------------------------------
							input_angle_h =  para[payload_len+110]
							input_angle_l =  para[payload_len+111]
							...
							for i=1,7,1 do	
								...
								diff_sum  = diff_sum  + diff_table[i]
							end															
						...
						if((l==payload_len+122) and (para[1]==bit32.band(para[6]+14+225,0xff))) then
							...
							rssi_table[8] = rssi
							angle_table[8] = input_angle
							therta_table[8] = para[payload_len+118]
...
-- Draw line
	...
	tl_form_draw_line(180,245,180,170,0xffff)
...
function change_line_for_triangle(angle,angle_old,pcolor)
	...
	if (angle_old >0 or angle_old ==0)	   then		--positive
		...
		tl_form_draw_line(180,340,x1,y1,pcolor+0x80ff)
...
function Draw_sub_interface(hDut)
local command = array.new(16)
	rst_alg(hDut)
	command[1] = 0x33 
	Send_Cmd_To_DUT(hDut,command,16)
...
function AOA_run_zhiwei(handle)

	Draw_sub_interface(handle)
	
	
end 
handle = Get_Dut_Handle(0xffff)
