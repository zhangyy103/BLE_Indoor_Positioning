...
function RF_PER_Run_AOA(hDut)
	...
	pkt_cnt = 0xffff	
	command[1] = 0x11 
	Send_Cmd_To_DUT_Through_USB(hDut,command,16)
	tl_sleep_ms(200)
	command[1] = 0x33 
	Send_Cmd_To_DUT_Through_USB(hDut,command,16)
	if(Is_Result_Empty_Judge_By_USB(hDut,1000)==1) then
		r, l = Read_Result_From_USB(handle , 32)
		chn = r[2] + 2400
		accesscod = bit32.bor(bit32.lshift (r[6], 24), bit32.lshift (r[5], 16),bit32.lshift (r[4], 8),r[3])
		board_type = r[7]
		IQ_data_len_8_bit_en = r[8]
		payload_len  = r[9]
	end
		...
		Send_Cmd_To_DUT_Through_USB(hDut,command,16)
			...
			do
				...
				if(pkt_cnt<recode_pkt_num)then
					para,l = Read_Result_From_USB(hDut , payload_len+122)						
					s= " "
					if(IQ_data_len_8_bit_en == 1)then 
					
						if((l==payload_len+122) and (para[1]==payload_len+104) ) then						
			...
			if board_type==1 then
				...
				do
					...
					if(IQ_data_len_8_bit_en == 1)then 
						...
						if((l==payload_len+122) and (para[1]==(bit32.band (payload_len+104,0xff)))) then
						...
						Send_Cmd_To_DUT_Through_USB(hDut,command,16)
						para,l = Read_Result_From_USB(hDut , payload_len+122)
						if((l==payload_len+122) and (para[1]==(bit32.band(payload_len+14+225,0xff)))) then
...
print(string.format("handle: %x",handle))
